...
Виконайте запропоновані кроки для відтворення проблеми
Зʼясуйте та виправіть причину першої проблеми з розгортанням мікросервісу
Знайдіть рішення для відкриття доступу до нової версії тільки для qa команди
Рекомендації:

Скористайтеся командами діагностики kubectl для отримання поточного стану проблемних ресурсів.
Застосуйте підхід canary deployment та функцію self configuration API-Gateway для розподілення трафіку
Використовуючи команду helm template підготуйте повноцінний робочий маніфест для розгортання нової версії мікросервісу
Відповіддю на завдання буде команда helm template з необхідними параметрами для розгортання нової версії мікросервісу.